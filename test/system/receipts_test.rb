require "application_system_test_case"

class ReceiptsTest < ApplicationSystemTestCase
  setup do
    @receipt = receipts(:one)
  end

  test "visiting the index" do
    visit receipts_url
    assert_selector "h1", text: "Receipts"
  end

  test "should create receipt" do
    visit receipts_url
    click_on "New receipt"

    fill_in "Barangay", with: @receipt.barangay
    fill_in "City", with: @receipt.city
    fill_in "End date", with: @receipt.end_date
    fill_in "Game", with: @receipt.game
    fill_in "House number", with: @receipt.house_number
    fill_in "Mobile number", with: @receipt.mobile_number
    fill_in "Name", with: @receipt.name
    fill_in "Payment method", with: @receipt.payment_method
    fill_in "Province", with: @receipt.province
    fill_in "Rent duration", with: @receipt.rent_duration
    fill_in "Rent rate", with: @receipt.rent_rate
    fill_in "Start date", with: @receipt.start_date
    fill_in "Sub total", with: @receipt.sub_total
    click_on "Create Receipt"

    assert_text "Receipt was successfully created"
    click_on "Back"
  end

  test "should update Receipt" do
    visit receipt_url(@receipt)
    click_on "Edit this receipt", match: :first

    fill_in "Barangay", with: @receipt.barangay
    fill_in "City", with: @receipt.city
    fill_in "End date", with: @receipt.end_date
    fill_in "Game", with: @receipt.game
    fill_in "House number", with: @receipt.house_number
    fill_in "Mobile number", with: @receipt.mobile_number
    fill_in "Name", with: @receipt.name
    fill_in "Payment method", with: @receipt.payment_method
    fill_in "Province", with: @receipt.province
    fill_in "Rent duration", with: @receipt.rent_duration
    fill_in "Rent rate", with: @receipt.rent_rate
    fill_in "Start date", with: @receipt.start_date
    fill_in "Sub total", with: @receipt.sub_total
    click_on "Update Receipt"

    assert_text "Receipt was successfully updated"
    click_on "Back"
  end

  test "should destroy Receipt" do
    visit receipt_url(@receipt)
    click_on "Destroy this receipt", match: :first

    assert_text "Receipt was successfully destroyed"
  end
end
